---------------------------------------------------------------
-- ACT Calculator Program for TI-Nspire
...
local menu = {
  ...
  {"Exponents & Logs",
    ...
    {"Solve Equation", function() selectTool("exp_solve", "Solve") end},
    ...
    {"Log Solver", function() selectTool("log_solve", "Log Solver") end},
    {"Change of Base", function() selectTool("log_cob", "Change Base") end},
  },
  {"Sequences & Series",
    {"Explicit Seq & Sum", function() selectTool("seq_explicit", "Explicit Seq") end},
    {"Recursive Seq & Sum", function() selectTool("seq_recursive", "Recursive Seq") end},
    {"Seq Formula Finder", function() selectTool("seq_finder", "Seq Finder") end},
    {"Geometric Seq/Series", function() selectTool("seq_geometric", "Geometric") end},
    {"Arithmetic Sequence", function() selectTool("seq_arith", "Arithmetic") end},
  },
  {"Matrices",
    {"Matrix A Explorer", function() selectTool("mat_explore", "Matrix A") end},
    {"A + B", function() selectTool("mat_add", "A + B") end},
    {"Inverse of A", function() selectTool("mat_inv", "A Inverse") end},
    {"Determinant of A", function() selectTool("mat_det", "det(A)") end},
    {"Row Echelon(A)", function() selectTool("mat_ref", "ref(A)") end},
    {"RREF(A)", function() selectTool("mat_rref", "rref(A)") end},
    {"Solve AX=B", function() selectTool("mat_axb", "AX=B") end},
    {"Cramer Rule AX=B", function() selectTool("mat_cramer", "Cramer") end},
  },
  {"Trigonometry",
    ...
    {"Read: 3 Laws", function() selectRead("trig_laws", "Trig Laws") end},
    {"SSS Triangle", function() selectTool("trig_sss", "SSS") end},
    {"SAS Triangle", function() selectTool("trig_sas", "SAS") end},
    {"SSA Triangle", function() selectTool("trig_ssa", "SSA") end},
    {"SAA Triangle", function() selectTool("trig_saa", "SAA") end},
  ...
  {"Circles",
    ...
    {"Find Sector", function() selectTool("circ_sector", "Sector") end},
    {"Arc Length", function() selectTool("circ_arc", "Arc Length") end},
  },
  {"Geometry",
    ...
    {"Rhombus", function() selectTool("geo_rhombus", "Rhombus") end},
  ...
  {"Convert",
    {"Polar <-> (x,y)", function() selectTool("conv_polar", "Polar") end},
    {"Degree <-> Radian", function() selectTool("conv_degrad", "Deg/Rad") end},
    {"Degree to DMS", function() selectTool("conv_dms", "DMS") end},
    {"Revolution to Deg/Rad", function() selectTool("conv_rev", "Revolution") end},
  },
  {"ACT Specials",
...
tools["seq_finder"] = {
  inputs = {{"Terms (comma sep)=", "", "e.g. 2,4,8,16"}},
  compute = function(v)
    local r = {}
    local terms = {}
    ...
    if terms[1] ~= 0 then
      ...
      if isGeo then
        ...
        table.insert(r, "a(n) = "..terms[1].." * "..ratio.."^(n-1)")
      end
    end
    return r
  end
...
tools["seq_geometric"] = {
  inputs = {{"a1=", "", "e.g. 3"}, {"r=", "", "e.g. 0.5"}, {"n=", "", "e.g. 10"}},
  compute = function(v)
    ...
    table.insert(r, "Geometric Sequence")
    table.insert(r, "(Multiply by r each step)")
...
tools["seq_arith"] = {
  ...
  compute = function(v)
    ...
    if a1 and a2 then
      ...
      table.insert(r, "")
      table.insert(r, "Arithmetic Sequence")
      table.insert(r, "(Add d each step)")
      table.insert(r, "Difference d="..d)
...
tools["mat_add"] = {
  inputs = {{"Matrix A=", "", "e.g. [1,2;3,4]"}, {"Matrix B=", "", "e.g. [3;7]"}},
  compute = function(v)
    local r = {}
    local s = safeEval("("..v[1]..")+("..v[2]..")")
...
tools["trig_sss"] = {
  ...
  compute = function(v)
    ...
    table.insert(r, "A="..round(A).." deg B="..round(B).." deg C="..round(C).." deg")
    local s = (a+b+c)/2
...
tools["trig_sas"] = {
  inputs = {{"a=", "", "e.g. 3"}, {"C (deg)=", "", "e.g. 90"}, {"b=", "", "e.g. 4"}},
  compute = function(v)
    ...
    local B = 180 - A - C
    table.insert(r, "SAS: a="..a.." C="..C.." deg b="..b)
    table.insert(r, "c="..round(c))
    table.insert(r, "A="..round(A).." deg B="..round(B).." deg")
...
tools["trig_ssa"] = {
  inputs = {{"a=", "", "e.g. 3"}, {"b=", "", "e.g. 4"}, {"A (deg)=", "", "e.g. 30"}},
  compute = function(v)
    ...
    table.insert(r, "SSA: a="..a.." b="..b.." A="..A.." deg")
    table.insert(r, "B="..round(B).." deg C="..round(C).." deg c="..round(c))
    ...
    if sinB < 1 and B < 90 then
      ...
      if C2 > 0 then
        ...
        table.insert(r, "B="..round(B2).." deg C="..round(C2).." deg c="..round(c2))
      end
    end
    return r
  end
...
tools["trig_saa"] = {
  inputs = {{"a=", "", "e.g. 3"}, {"A (deg)=", "", "e.g. 30"}, {"B (deg)=", "", "e.g. 60"}},
  compute = function(v)
    ...
    table.insert(r, "SAA: a="..a.." A="..A.." deg B="..B.." deg")
    table.insert(r, "C="..round(C).." deg b="..round(b).." c="..round(c))
...
geoTool("geo_rhombus", {{"d1=", "", "e.g. 6"}, {"d2=", "", "e.g. 8"}}, function(v)
  ...
  table.insert(r, "Area = 1/2d1*d2 = "..round(0.5*d1*d2))
...
tools["conv_polar"] = {
  ...
  compute = function(v)
    ...
    table.insert(r, "theta = "..round(theta).." deg = "..round(math.rad(theta)).." rad")
    table.insert(r, "")
    table.insert(r, "Polar: ("..round(radius)..", "..round(theta).." deg)")
    return r
  end
}
...
tools["conv_dms"] = {
  ...
  compute = function(v)
    ...
    table.insert(r, d.." deg = "..deg.." deg "..min.."' "..sec.."\"")
    return r
  end
}

tools["conv_rev"] = {
  inputs = {{"Revolutions=", "", "e.g. Revolutions"}},
  compute = function(v)
    local rev = tonumber(v[1])
    local r = {}
    if not rev then table.insert(r, "Enter revolutions") return r end
    table.insert(r, rev.." rev = "..round(rev*360).." deg")
